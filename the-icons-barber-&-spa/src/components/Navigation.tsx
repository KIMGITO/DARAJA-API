...
export const Navigation: React.FC = () => {
  ...
  const navLinks = [
    ...
    { label: 'Services & Pricing', path: '/services', desc: 'Precision Fades, Hot Shaves & Spa' },
    { label: 'Grooming Products', path: '/products', desc: 'Must-Have Tonics, Shampoos & Oils' },
    { label: 'Master Barbers', path: '/barbers', desc: 'Certified Grooming Artisans' },
    { label: 'About The Sanctuary', path: '/about', desc: 'Our Philosophy & Standards' },
    { label: 'Grooming Gallery', path: '/gallery', desc: 'Executive Portfolio & Showcase' },
    { label: 'FAQ', path: '/faq', desc: 'Appointments, Policies & Questions' },
    { label: 'Location & Contact', path: '/contact', desc: 'Visit our penthouse studio in Nairobi' }
  ];
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            {/* Center: Luxury Cursive Script Logo with Gold Flourish (like reference image) */}
            <button 
              id="brand-logo-btn"
              onClick={() => handleNavClick('/')}
              className="flex flex-col items-center justify-center text-center group focus:outline-none py-1"
              aria-label="The Icons Barber and Spa Home"
            >
              <div className="relative flex flex-col items-center">
                {/* Script Title */}
                <div className="flex items-center gap-1">
                  <span className="font-script text-2xl sm:text-3xl md:text-4xl text-white tracking-wide leading-none select-none drop-shadow-sm">
                   <span className="text-primary"> The </span>  Icons
                  </span>
                </div>

                {/* Decorative Gold Scissors Line */}
<div className="w-20 sm:w-24 h-2 relative -mt-1 pointer-events-none flex items-center justify-center">
  <svg 
    viewBox="0 0 100 24" 
    fill="none" 
  ...
  >
    ...
    <circle cx="15" cy="12" r="1.5" fill="currentColor"/>
                ...
                {/* Subtitle */}
                <span className="text-[9px] sm:text-[10px] tracking-[0.22em] text-foreground/80 group-hover:text-white transition-colors uppercase font-normal -mt-0.5">
                  BarberShop & Spa
                </span>
              </div>
            </button>
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <button
                ...
                className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-primary/30 bg-primary/10 hover:bg-primary/20 text-primary text-xs font-semibold transition-colors"
                title="Open Staff & Artisan Portal"
              >
            ...
            <div>
              <div className="flex items-center justify-between pb-6 border-b border-border">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-white font-heading text-sm font-bold tracking-wider">THE ICONS</h2>
                    <p className="text-[10px] text-primary tracking-widest uppercase">Barber & Spa • Nairobi</p>
                  </div>
                </div>
                ...
                {navLinks.map((link) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <ChevronRight className={`w-4 h-4 transition-transform group-hover:translate-x-1 ${isActive ? 'text-primary' : 'text-muted-foreground'}`} />
                    </button>
                  );
                })}
              </nav>
            ...
            <div className="pt-6 border-t border-border space-y-4">
              ...
              <div className="space-y-2 text-xs text-muted-foreground-light">
                ...
                >
                  ...
                  <span>Call Concierge: <strong className="text-white">{businessInfo.phoneDisplay}</strong></span>
                </a>

                <div className="flex items-center gap-2.5 p-2 text-muted-foreground">
                  <MapPin className="w-3.5 h-3.5 text-primary" />
                  <span>{businessInfo?.address?.suite}, {businessInfo?.address?.street}</span>
                </div>
