  ...
  return (
    ...
    >
            ...
            >
              <div className="flex items-center bg-white/95 backdrop-blur-md rounded-xl p-1 shadow-lg border border-white/30 focus-within:border-primary transition-all">
                <input
                  ...
                  required
                  className="w-full bg-transparent text-black placeholder:text-muted-foreground text-xs sm:text-sm px-3 py-1.5 focus:outline-none min-w-0 font-sans"
                />
                <Button
                  type="submit"
                  variant="primary"
      ...
      {showLegalModal && (
        ...
        >
          <div
            className="bg-card/90 backdrop-blur-2xl border border-white/20 rounded-3xl max-w-lg w-full p-6 text-left relative shadow-2xl"
          ...
          >
            ...
            <p className="text-xs text-muted-foreground-light leading-relaxed mb-6 font-light">
              {showLegalModal === 'terms'
                ? 'By scheduling a service at The Icons Barber & Spa, you agree to our 2-hour rescheduling policy. Cancellations can be managed online or via WhatsApp. All treatments prioritize hygienic executive sanctuary standards.'
                : 'The Icons respects client confidentiality. Your contact and appointment records are encrypted and strictly used for confirmation notices, bespoke loyalty offerings, and booking reminders.'}
            </p>
            <Button
              variant="primary"
              size="md"
              onClick={() => setShowLegalModal(null)}
              className="w-full text-xs uppercase tracking-wider rounded-xl shadow-lg"
            >
              Close
            </Button>
          </div>
